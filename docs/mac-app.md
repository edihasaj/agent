...
- Pick bundle id: `com.edihasaj.<appname>` (no uppercase, no spaces).
- Appcast URL: `https://raw.githubusercontent.com/edihasaj/<Repo>/main/appcast.xml`
...
- Add to `Scripts/`:
  ...
  - `release.sh` sourcing `~/Projects/agent/release/sparkle_lib.sh` to: lint/test, sign+notarize, clear caches, verify appcast/enclosure, optional live-update test, create GH release, check assets, tag/push.
