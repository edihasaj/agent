 ...
 *
 * Scope:
 *   - per-project: ~/.abx/projects/<slug>/learnings.jsonl
 *   - global:      ~/.abx/global-domain-skills.jsonl
 *
...
function globalFile(): string {
  return path.join(abxHome(), 'global-domain-skills.jsonl');
}

function projectFile(slug: string): string {
  return path.join(abxHome(), 'projects', slug, 'learnings.jsonl');
}
