 ...
 * and passes it via ABX_SKILL_TOKEN. The skill never sees the daemon root
 ...
 *    ABX_PORT and ABX_SKILL_TOKEN are always set.
 ...
 * Includes: minimal PATH, locale, terminal type. Skills get ABX_PORT +
 * ABX_SKILL_TOKEN injected separately.
 */
...
export function buildSpawnEnv(opts: BuildEnvOptions): Record<string, string> {
  ...
  if (opts.trusted) {
    ...
    for (const [k, v] of Object.entries(process.env)) {
      ...
      if (k === 'ABX_TOKEN') continue; // never propagate root token
      out[k] = v;
    }
  ...
  // intentionally — e.g. an internal-tool skill — but we still strip ABX_TOKEN
  // above.)
  if (!opts.trusted) {
  ...
  out.ABX_PORT = String(opts.port);
  out.ABX_SKILL_TOKEN = opts.skillToken;
