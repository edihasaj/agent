 ...
 *
 * Auth resolution:
 *   1. ABX_PORT + ABX_SKILL_TOKEN env vars (set by `$B skill run` when
 ...
 *   2. State file fallback: read `BROWSE_STATE_FILE` env or `<git-root>/.abx/browse.json`
...
export interface BrowseClientOptions {
  /** Override port. Default: ABX_PORT env or state file. */
  port?: number;
  /** Override token. Default: ABX_SKILL_TOKEN env, then state file root token. */
  token?: string;
  /** Tab id to target (every command can scope to a tab). Default: BROWSE_TAB env or undefined (active tab). */
  tabId?: number;
  /** Per-request timeout in milliseconds. Default: 30_000. */
  timeoutMs?: number;
  /** Override state-file path. Default: BROWSE_STATE_FILE env or <git-root>/.abx/browse.json. */
  stateFile?: string;
}
...
export function resolveBrowseAuth(opts: BrowseClientOptions = {}): ResolvedAuth {
  ...
  const envPort = process.env.ABX_PORT;
  const envToken = process.env.ABX_SKILL_TOKEN;
  if (envPort && envToken) {
  ...
  throw new Error(
    ...
    '(sets ABX_PORT + ABX_SKILL_TOKEN) or run from a project with a live daemon ' +
    '(.abx/browse.json must exist).'
  );
}
...
function defaultStateFile(): string | null {
  try {
    ...
    return path.join(base, '.abx', 'browse.json');
  } catch {
    return path.join(process.cwd(), '.abx', 'browse.json');
  }
}
