...
export async function handleMetaCommand(
  ...
  switch (command) {
    ...
    case 'restart': {
      ...
      console.log('[abx] Restart requested. Exiting for CLI to restart.');
      await shutdown();
      return 'Restarting...';
    }
    ...
    case 'disconnect': {
      ...
      console.log('[abx] Disconnecting headed browser. Restarting in headless mode.');
      await shutdown();
    ...
    case 'state': {
      ...
      if (action === 'load') {
        ...
        if (validatedCookies.length < data.cookies.length) {
          console.warn(`[abx] Filtered ${data.cookies.length - validatedCookies.length} invalid cookies from state file`);
        }
        ...
        if (data.savedAt) {
          ...
          if (ageMs > SEVEN_DAYS) {
            console.warn(`[abx] Warning: State file is ${Math.round(ageMs / 86400000)} days old. Consider re-saving.`);
          }
        }
