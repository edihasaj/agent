 ...
 *   project:  <project>/.abx/browser-skills/<name>/
 *   global:   ~/.abx/browser-skills/<name>/
 *   bundled:  <abx-install>/browser-skills/<name>/   (read-only, ships with abx)
 *
...
export function defaultTierPaths(opts: { projectRoot?: string; home?: string; bundledRoot?: string } = {}): TierPaths {
  ...
  return {
    project: projectRoot ? path.join(projectRoot, '.abx', 'browser-skills') : null,
    global: path.join(home, '.abx', 'browser-skills'),
...
function detectBundledRoot(): string {
  // The browse binary lives at <abx-install>/browse/dist/browse.
  // The bundled browser-skills/ dir is a sibling of browse/ (i.e. <abx-install>/browser-skills/).
  ...
  // browse/src/browser-skills.ts → ../../  (the abx root).
 ...
 *
 * Cannot tombstone bundled skills — they ship with abx and are read-only.
 * To remove a bundled skill, override it with a global/project entry, or
 * remove the file from the abx source tree.
 */
