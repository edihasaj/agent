 ...
 *
 * Piggybacks on ~/.abx/analytics/skill-usage.jsonl pattern (existing
 * abx telemetry). Hostname + aggregate counters only; no body content,
 * no agent text, no command args. Respects the user's telemetry tier
 * setting (off | anonymous | community) via abx-config.
 *
...
function abxHome(): string {
  return process.env.ABX_HOME || stateRoot();
}

function analyticsDir(): string {
  return path.join(abxHome(), 'analytics');
}
...
function isDisabled(): boolean {
  ...
  if (process.env.ABX_TELEMETRY_OFF === '1') {
    telemetryDisabled = true;
    return true;
  }
  // Conservative default: telemetry ON unless explicitly off. Users opt out via
  // abx-config set telemetry off (preamble reads this; we trust the env hint).
