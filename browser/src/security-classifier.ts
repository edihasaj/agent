 ...
 *
 * To upgrade: bump this string, run `ABX_BENCH_ENSEMBLE=1 bun test
 ...
 * layout at ~/.abx/models/testsavant-small/ on first use.
 *
 ...
 */
const MODELS_DIR = path.join(stateRoot(), 'models');
...
// ABX_SECURITY_ENSEMBLE=deberta. Not forced on every install because
...
function isDebertaEnabled(): boolean {
  const setting = (process.env.ABX_SECURITY_ENSEMBLE ?? '').toLowerCase();
...
export function loadTestsavant(onProgress?: (msg: string) => void): Promise<void> {
  if (process.env.ABX_SECURITY_OFF === '1') {
    testsavantState = 'failed';
    testsavantLoadError = 'ABX_SECURITY_OFF=1 — ML classifier kill switch engaged';
...
export function loadDeberta(onProgress?: (msg: string) => void): Promise<void> {
  if (process.env.ABX_SECURITY_OFF === '1') return Promise.resolve();
...
export async function checkTranscript(params: {
  ...
  return new Promise((resolve) => {
    ...
    // repo with a prompt-injection-defense CLAUDE.md (like abx itself),
    ...
    // Env var ABX_HAIKU_TIMEOUT_MS (milliseconds) overrides for benches
    // that want a different budget.
    const timeoutMs = process.env.ABX_HAIKU_TIMEOUT_MS
      ? Number(process.env.ABX_HAIKU_TIMEOUT_MS)
      : 45000;
    setTimeout(() => {
