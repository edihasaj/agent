/**
 * Terminal Agent — PTY-backed Claude Code terminal for the abxr
...
const STATE_FILE = process.env.BROWSE_STATE_FILE || path.join(stateRoot(), 'browse.json');
 ...
 *      (updated continuously by the abxr extension).
...
function buildTabAwarenessHint(stateDir: string): string {
  ...
  return [
    'You are running inside the abxr sidebar with live access to the user\'s browser tabs.',
    '',
...
function spawnClaude(cols: number, rows: number, onData: (chunk: Buffer) => void) {
  ...
  // headed-mode browser; BROWSE_NO_AUTOSTART prevents claude's abx
...
function buildServer() {
  return Bun.serve({
    ...
    fetch(req, server) {
      ...
      //         - Cookie abx_pty (works for non-browser callers and
      ...
      if (url.pathname === '/ws') {
        ...
        // with a `abx-pty.` prefix (which we strip). Browsers send a
        ...
        for (const raw of protoHeader.split(',').map(s => s.trim()).filter(Boolean)) {
          const candidate = raw.startsWith('abx-pty.') ? raw.slice('abx-pty.'.length) : raw;
        ...
        // Fallback: Cookie abx_pty (legacy / non-browser callers).
        if (!token) {
