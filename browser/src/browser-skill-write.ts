 ...
 *
 * /skillify stages a candidate skill into ~/.abx/.tmp/skillify-<spawnId>/,
 ...
 *   <tmpRoot>/.abx/.tmp/skillify-<spawnId>/<name>/
 *
...
export function stageSkill(opts: StageSkillOptions): string {
  ...
  const tmpRoot = opts.tmpRoot ?? path.join(stateRoot(), '.tmp');
